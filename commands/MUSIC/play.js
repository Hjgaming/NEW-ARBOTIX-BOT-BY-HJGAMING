...
run:async (client, message, args)=> {
  ...
  if (!channel) {
    let embed = new MessageEmbed()
      .setColor("428CCF")
      .setDescription(`⚠️ Please connect with voice channel`)
  ...
  if (!args.length) {
    let embed = new MessageEmbed()
      .setColor("428CCF")
      .setDescription(`⚠️ Please write song or give me link`)
    return message.channel.send(embed)
  }
  ...
  if (!play) {
    ...
    if (!channel.joinable) {
      let embed = new MessageEmbed()
        ...
        .setDescription("⚠️ I can't able to join your voice channel")
      return message.channel.send(embed)
    }
  ...
  if (player.options.voiceChannel !== channel.id) {
    let embed = new MessageEmbed()
      .setColor("428CCF")
      .setDescription("⚠️ you must join ${channel.name}"
  ...
  try {
    ...
    let embed = new MessageEmbed()
      ...
      .setDescription(`⚠️ i can't find result about \`${search}\``
       )
    return message.channel.send(embed)
  }
  ...
  switch (res.loadType) {
    case 'NO_MATCHES':
      ...
      let embed = new MessageEmbed()
        ...
        .setDescription(`⚠️ i can't find results about \`${search}\``)
      return message.channel.send(embed)
